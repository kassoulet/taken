...
test.describe("Multi-Registry Package Name Checker", () => {
  ...
  test("should display the main page with title", async ({ page }) => {
    ...
    await expect(
      page.locator('h2:has-text("Multi-Registry Package Name Checker")'),
    ).toBeVisible();

    // Check that the input field is present
    await expect(
      page.locator('input[aria-label="Package name"]'),
    ).toBeVisible();
  });
  ...
  test("should show results after entering a package name", async ({
    page,
  }) => {
    const input = page.locator('input[aria-label="Package name"]');
  ...
  test("should handle 404 responses correctly (package name not taken)", async ({
    page,
  }) => {
    const input = page.locator('input[aria-label="Package name"]');
